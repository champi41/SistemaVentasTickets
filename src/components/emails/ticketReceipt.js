...
function expandPerTicket(items, purchase) {
  const seed = String(purchase?.id || purchase?._id || purchase?.reservation_id || "ORDER")
    .slice(-6).toUpperCase();

  const rows = [];
  items.forEach((it) => {
    for (let i = 0; i < it.quantity; i++) {
      const code = it.codes?.[i] ||
        `${norm(it.type).replace(/\s+/g, "").toUpperCase()}-${String(i + 1).padStart(2, "0")}-${seed}`;
      rows.push({
        idx: rows.length + 1,
        type: it.type,
        price: it.unitPrice,
...
export function renderTicketEmail({ purchase = {}, event = {}, toName = "", reservationItems = [] }) {
  ...
  const total = (purchase?.total_price != null) ? Number(purchase.total_price) : computedTotal;

  const eventDate = event?.date
    ? new Date(event.date).toLocaleString("es-CL", { dateStyle: "full", timeStyle: "short" })
    : "Fecha por confirmar";
  ...
  const itemsRows = items.length
    ? items.map(it => `
      <tr>
        ...
        <td style="padding:10px 12px;border:1px solid #e5e7eb;text-align:center;">${it.quantity}</td>
        <td style="padding:10px 12px;border:1px solid #e5e7eb;text-align:right;">$${fmtCLP(it.unitPrice)}</td>
        <td style="padding:10px 12px;border:1px solid #e5e7eb;text-align:right;">$${fmtCLP(it.subtotal)}</td>
      </tr>
    `).join("")
  ...
  const ticketRows = perTicket.length
    ? perTicket.map(t => `
      <tr>
        ...
        <td style="padding:10px 12px;border:1px solid #e5e7eb;text-align:right;">$${fmtCLP(t.price)}</td>
        <td style="padding:10px 12px;border:1px solid #e5e7eb;font-family:ui-monospace,Menlo,Consolas,monospace;">${t.code}</td>
      </tr>
    `).join("")
    ...
    <table role="presentation" width="100%" style="max-width:720px;margin:0 auto;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 8px 26px rgba(17,24,39,.08);">
      <tr>
        <td style="padding:20px 24px;background:#111827;color:#fff;">
          ...
          <div style="font-size:12px;opacity:.9;">${new Date().toLocaleDateString("es-CL")}</div>
        </td>
      </tr>

      ${event?.image ? `
      <tr><td>
        <img src="${event.image}" alt="${event?.name || "Evento"}" style="display:block;width:100%;max-height:260px;object-fit:cover;">
      </td></tr>` : ""}
      ...
      <tr><td style="padding:16px 24px 0 24px;">
        <table role="presentation" width="100%" style="border:1px solid #e5e7eb;border-radius:12px;">
          <tr><td style="padding:14px 16px;">
            <div style="font-weight:600;font-size:16px;color:#111827;">${event?.name || "Evento"}</div>
            <div style="font-size:14px;color:#374151;">Fecha: ${eventDate}</div>
            <div style="font-size:14px;color:#374151;">Lugar: ${event?.location || "Por confirmar"}</div>
            ${event?.category ? `<div style="font-size:13px;color:#6b7280;margin-top:6px;">Categoría: ${event.category}</div>` : ""}
          </td></tr>
        </table>
      </td></tr>
        ...
        <table role="presentation" width="100%" style="border-collapse:collapse;">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="padding:12px;border:1px solid #e5e7eb;text-align:right;font-weight:700;">$${fmtCLP(total)} CLP</td>
            </tr>
          </tbody>
        </table>
      </td></tr>
